...
// XIZMATLARNI YUKLASH (Hamma xizmatlar to'liq yuklanadi)
async function loadServices() {
    ...
    if (Array.isArray(data)) {
        ...
        console.log(`✅ Tizimga ${ALL_SERVICES.length} ta xizmat va ${CATEGORIES.length} ta kategoriya yuklandi!`);
    } else {
        console.log("⚠️ Xizmatlarni yuklashda xatolik yuz berdi.");
    }
}
...
// 🚀 /START BUYRUG'I (To'liq to'g'rilandi)
bot.onText(/\/start(.*)/, async (msg, match) => {
    ...
    try {
        ...
        if (!dbUser) {
            dbUser = new User({ userId, balance: 0, refCount: 0 });
            await dbUser.save();
            await Stats.updateOne({}, { $inc: { users: 1 } }, { upsert: true });
            
            if (refId && refId != userId) {
                const checkRef = await User.findOne({ userId: parseInt(refId) });
                if (checkRef) {
...
bot.onText(/\/addbalans (\d+) (\d+)/, async (msg, match) => {
    ...
    bot.sendMessage(msg.chat.id, `✅ Yangi balans kiritildi!`);
...
bot.on('message', async (msg) => {
    ...
    if (msg.photo && userStep[chatId] && userStep[chatId].step === 'chek_kutish') {
        ...
        bot.sendMessage(chatId, `✅ To'lov cheki qabul qilindi. Tez orada adminga yuboriladi.`);
        delete userStep[chatId]; 
    ...
    if (text === '👤 Admin panel' && userId === ADMIN_ID) {
        return bot.sendMessage(chatId, `👨‍💻 Admin buyruqlari:\n\n\`/addbalans [ID] [SUMMA]\``, {parse_mode: 'Markdown', reply_markup:{keyboard:[['🔙 Orqaga']],resize_keyboard:true}});
    }

    if (text === '💰 Hisobni to‘ldirish') {
        userStep[chatId] = { step: 'summa_kiritish' };
        return bot.sendMessage(chatId, `💰 Qancha so'm kiritmoqchisiz? Faqat raqam kiriting:`);
    }

    if (userStep[chatId] && userStep[chatId].step === 'summa_kiritish') {
        const { summa } = { summa: parseInt(text) };
        if (isNaN(summa) || summa < 1000) return bot.sendMessage(chatId, `❌ Kamida 1000 so'm yozing:`);
        userStep[chatId] = { step: 'chek_kutish', summa: summa };
        return bot.sendMessage(chatId, `💳 Karta raqami: \`${CARD_NUMBER}\`\n👤 Ega: ${CARD_NAME}\n💰 To'lov summasi: ${summa} so'm\n\nTo'lovni bajarib, chekni rasm shaklida yuboring!`, { parse_mode: 'Markdown' });
    }

    // 🛒 SMM XIZMATLAR KATEGORIYALARI (To'g'rilandi - hamma kategoriyalar chiqadi)
    if (text === '🛒 SMM Xizmatlar') {
        if (CATEGORIES.length === 0) return bot.sendMessage(chatId, "⚠️ Xizmatlar bazasi yangilanmoqda, qayta urining...");
        
        let inline_keyboard = [];
        CATEGORIES.forEach((cat, idx) => {
            inline_keyboard.push([{ text: cat, callback_data: `cat_${idx}` }]);
        });

        return bot.sendMessage(chatId, `📋 *Kerakli SMM bo'limini tanlang:*`, { 
            parse_mode: 'Markdown',
    ...
    if (userStep[chatId] && userStep[chatId].step === 'sel_serv') {
        ...
        if (!service) return bot.sendMessage(chatId, "❌ Noto'g'ri ID. Qayta kiriting:");
        userStep[chatId].service = service;
        userStep[chatId].step = 'send_link';
        return bot.sendMessage(chatId, `🔗 *${service.name}* tanlandi.\n\nBuyurtma havolasini (Link) kiriting:`, { parse_mode: 'Markdown' });
    }

    if (userStep[chatId] && userStep[chatId].step === 'send_link') {
        userStep[chatId].link = text;
        userStep[chatId].step = 'send_count';
        return bot.sendMessage(chatId, `📦 Miqdorni kiriting (Min: ${userStep[chatId].service.min} / Max: ${userStep[chatId].service.max}):`);
    }

    if (userStep[chatId] && userStep[chatId].step === 'send_count') {
        const cnt = parseInt(text);
        const s = userStep[chatId].service;
        if (isNaN(cnt) || cnt < parseInt(s.min) || cnt > parseInt(s.max)) return bot.sendMessage(chatId, `❌ Noto'g'ri miqdor! Qayta kiriting:`);

        const ClientRate = parseFloat(s.rate) * PROFIT_PERCENT;
        const sum = Math.ceil((cnt / 1000) * ClientRate);
        const dbUser = await User.findOne({ userId });

        if (!dbUser || dbUser.balance < sum) {
            delete userStep[chatId];
            return bot.sendMessage(chatId, `❌ Balans yetarli emas. Narxi: ${sum} so'm. Sizda: ${dbUser ? dbUser.balance : 0} so'm`);
        }

        bot.sendMessage(chatId, `⏳ Buyurtma tizimga yo'llanmoqda...`);
        ...
        if (result && result.order) {
            ...
            bot.sendMessage(chatId, `❌ Tizim rad etdi: ${result ? result.error : 'Nomalum xatolik'}`);
        }
        delete userStep[chatId];
        return;
    }
    ...
    if (text === '📦 Buyurtmalarim') {
        const myOrders = await Order.find({ userId }).sort({_id: -1}).limit(10);
        if (myOrders.length === 0) return bot.sendMessage(chatId, '❌ Sizda hozircha buyurtmalar mavjud emas.');
        let txt = '📦 OXIRGI BUYURTMALARINGIZ:\n\n';
        myOrders.forEach(o => { txt += `🆔 \`${o.orderId}\` | ${o.serviceName.slice(0, 20)}... | *${o.status}*\n`; });
        return bot.sendMessage(chatId, txt, { parse_mode: 'Markdown' });
    }

    if (text === '🔗 Referal havola') {
        const dbUser = await User.findOne({ userId });
        return bot.sendMessage(chatId, `🔗 https://t.me/SMM_adminMAX_bot?start=${userId}\n👥 Taklif etilgan do'stlar: ${dbUser ? dbUser.refCount : 0} ta`);
    }
});

// 🔄 INLINE KLAVIATURA TUGMALARI (Kategoriyalar va xizmatlar to'liq ochiladi)
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('cat_')) {
        ...
        filteredServices.slice(0, 25).forEach((s) => {
            const ClientRate = (parseFloat(s.rate) * PROFIT_PERCENT).toFixed(0);
            txt += `🆔 \`${s.service}\` — ${s.name}\n💰 Narxi: ${ClientRate} so'm (1000ta uchun)\n📦 Min: ${s.min} / Max: ${s.max}\n\n`;
        });
        txt += `✍️ Sotib olmoqchi bo'lgan xizmat *ID raqamini* yozib yuboring:`;
